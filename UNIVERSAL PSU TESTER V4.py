...
pg.setConfigOptions(useOpenGL=False, antialias=False) # DirectX tomará el control en Windows
...
web_data = {"V": 0.0, "A": 0.0, "W": 0.0, "Title": "UNIVERSAL PSU V4.0", "TimeWindow": 15}
user_lang = 'ES'
        ...
        if self.path == '/':
            ...
            <body>
                <div class="container" id="main-container">
                    <h1 id="web-title">UNIVERSAL PSU V4.0</h1>
                    ...
                    <div class="watermark">Youtube JlTexh8 Tester V4.0</div>
                </div>
                <script>
...
def start_web_server_thread(port=8080):
    ...
    try:
        ...
        if port < 8090: start_web_server_thread(port + 1)
...
class JlTexhMonitor(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        self.lbl_title = OverlayLabel("UNIVERSAL PSU V4.0", "#FFFFFF", 22, 0.5, 10, main_widget)
        ...
        self.lbl_ref_name = OverlayLabel("", "#00FF9C", 16, 0.5, 100, main_widget)
        ...
        watermark = QtWidgets.QLabel("Youtube JlTexh8 Tester V4.0", graph_container)
    ...
    def force_redraw_ref(self):
        if len(self.ref_data[0]) > 0:
            ...
            if len(t_ord) > 0:
                t_now = t_ord[-1] 
                ref_t_relative = self.ref_data[0] - t_now + self.ref_time_offset
                self.curve_ref.setData(ref_t_relative, self.ref_data[1])
    ...
    def update_data(self):
        ...
        if self.stream.new_data_ready:
            ...
            if len(t_ord) > 0:
                ...
                if len(self.ref_data[0]) > 0:
                    ref_t_relative = self.ref_data[0] - t_ord[-1] + self.ref_time_offset
                    self.curve_ref.setData(ref_t_relative, self.ref_data[1])
    ...
    def execute_save_ref(self, path, ref_name):
        ...
        if len(t_ord) > 0:
            ...
            save_t = t_ord[window_mask]
            save_c = c_ord[window_mask]
            
            if len(save_c) > 0:
                t_rel_save = save_t - save_t[0] 
            else:
    ...
    def load_ref(self):
        ...
        if dialog.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            path, _ = QtWidgets.QFileDialog.getOpenFileName(self, t('load_title'), "", "CSV (*.csv)")
            if path:
                try:
    ...
    def auto_diagnostico(self):
        ...
        error_medio = np.mean(np.abs(y_ref[-compare_len:] - y_curr[-compare_len:]))
